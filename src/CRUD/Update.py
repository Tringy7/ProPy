...
def update_record(df):
    ...
    country_to_update = input("Nhập tên quốc gia cần cập nhật: ")
    ...
    if country_to_update in df['Country/Region'].values:
        ...
        print(f"Dữ liệu của quốc gia '{country_to_update}' đã được cập nhật thành công!")
    else:
        print(f"Quốc gia '{country_to_update}' không tồn tại trong DataFrame.")
